...
Writing 2 to keycodetype switches the driver to pure gamepad mode. Only BTN_GAMEPAD is emitted and the KEY_SELECT injection stops entirely.

The sysfs node is world-writable, so no root is required to write to it — but the value resets to 0 on every boot because it lives in kernel memory, not persistent storage.
